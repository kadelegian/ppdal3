...
class Transaksi_pengeluaran extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($awal_periode <> '' && $sampai_dengan <> '') {
            ...
            $config['base_url'] = base_url() . 'transaksi_pengeluaran/index?awal_periode=' . $awal_periode . '&akhir_periode=' . $sampai_dengan;
            $config['first_url'] = base_url() . 'transaksi_pengeluaran/index?awal_periode=' . $awal_periode . '&akhir_periode=' . $sampai_dengan;
        } else {
        ...
        $config['total_rows'] = $this->transaksi_pengeluaran_model->total_rows($start_date, $end_date);
        $transaksi = $this->transaksi_pengeluaran_model->get_limit_data($config['per_page'],  $start_date, $end_date, $start);
    ...
    public function create()
    {
        ...
        $data_kas = $this->akun_model->get_akun_pettycash();
        $akun_tujuan = $this->akun_model->get_akun_tujuan_pengeluaran();
    ...
    public function create_action()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $id_trx = $this->transaksi_pengeluaran_model->insert($data);
            $data = array(
                array(
                    'tanggal' => date('Y-m-d h:i:s'),
                    ...
                    'id_transaksi_pengeluaran' => $id_trx,
                ), array(
                    'tanggal' => date('Y-m-d h:i:s'),
